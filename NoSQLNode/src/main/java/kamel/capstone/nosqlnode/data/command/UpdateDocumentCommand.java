...
public class UpdateDocumentCommand implements Command {
    ...
    public UpdateDocumentCommand(String command, DataAccessLayer dal, List<UserRole> userRole) throws AccessDeniedException {
        ...
        if (matcher.find()) {
            try {
                ...
                if (matcher.group(3) != null) {
                    ...
                    return dal.update(collectionName, attributes, List.of(new Condition(key, ConditionType.EQUAL, conditions.get(key))));
                } else {
                    return dal.update(collectionName, attributes, Collections.emptyList());
                }
